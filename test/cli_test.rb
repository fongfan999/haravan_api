...
class CliTest < Test::Unit::TestCase
  ...
  test "add with blank domain" do
    ...
    @cli.expects(:puts).with("\nopen https://foo.myharavan.com/admin/apps/private in your browser to create a private app and get API credentials\n")
    @cli.expects(:puts).with("Default connection is foo")

    @cli.add('foo')

    config = YAML.load(File.read(config_file('foo')))
    assert_equal 'foo.myharavan.com', config['domain']
  ...
  test "add with explicit domain" do
    standard_add_shop_prompts
    $stdin.expects(:gets).times(4).returns("bar.myharavan.com", "key", "pass", "y")
    @cli.expects(:puts).with("\nopen https://bar.myharavan.com/admin/apps/private in your browser to create a private app and get API credentials\n")
    @cli.expects(:puts).with("Default connection is foo")

    @cli.add('foo')

    config = YAML.load(File.read(config_file('foo')))
    assert_equal 'bar.myharavan.com', config['domain']
  end

  test "add with irb as shell" do
    standard_add_shop_prompts
    $stdin.expects(:gets).times(4).returns("bar.myharavan.com", "key", "pass", "fuuuuuuu")
    @cli.expects(:puts).with("\nopen https://bar.myharavan.com/admin/apps/private in your browser to create a private app and get API credentials\n")
    @cli.expects(:puts).with("Default connection is foo")
  ...
  def valid_options
    {'domain' => 'snowdevil.myharavan.com', 'api_key' => 'key', 'password' => 'pass', 'shell' => 'pry', 'protocol' => 'https'}
  end
  ...
  def standard_add_shop_prompts
    ...
    $stdout.expects(:print).with("Domain? (leave blank for foo.myharavan.com) ")
