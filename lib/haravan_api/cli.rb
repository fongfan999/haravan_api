  ...
  class Cli < Thor
    ...
    def add(connection)
      ...
      if File.exist?(file)
        ...
        config['domain']   = ask("Domain? (leave blank for #{connection}.myharavan.com)")
        config['domain']   = "#{connection}.myharavan.com" if config['domain'].blank?
        config['domain']   = "#{config['domain']}.myharavan.com" unless config['domain'].match(/[.:]/)
    ...
    def console(connection=nil)
      ...
      HaravanAPI::Base.site = site_from_config(config)
    ...
    def shop_config_dir
      @shop_config_dir ||= File.join(ENV['HOME'], '.haravan', 'shops')
    end
    ...
    def site_from_config(config)
      ...
      HaravanAPI::Base.site = "#{protocol}://#{api_key}:#{password}@#{domain}/admin"
    end
